...
from model.dataset import ClaimDataset, LABEL2ID, ID2LABEL
...
def evaluate():
    ...
    if es_mask.sum() > 0:
        ...
        print(f"  {es_results['per_class']}")
        gap = abs(en_results["f1_macro"] - es_results["f1_macro"])
        print(f"\nEN vs ES gap: {gap:.4f}")
    else:
        print("Test set is EN-only (LIAR). ES evaluation in Block I (sample_claims.json).")
